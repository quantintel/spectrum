...
/**
 *
 * 1 = do not round: return the number unmodified
 *
 * 2 = the first decimal place past the precision will be
 ...
 *
 * 3 = All decimal places past the precision will be truncated
 *
 * 4 = The first decimal place past the precision
 * will be rounded up if greater than or equal
 * to the rounding digit; this corresponds
 * to the OMG round-up rule.
 *
 * 5 = positive numbers will be rounded up and negative
 * numbers will be rounded down using the OMG round up
 * and round down rules
 *
 * 6 = positive numbers will be rounded down and negative
